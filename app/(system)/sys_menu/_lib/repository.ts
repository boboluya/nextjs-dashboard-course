...
export async function selectMenus(sysMenu: PageParams): Promise<SysMenu[]> {
  const conditions = [
    ...
    sysMenu.name
      ? like(sys_menuTable.name, `%${sysMenu.name}%`)
      : undefined,
    sysMenu.label
      ? like(sys_menuTable.label, `%${sysMenu.label}%`)
      : undefined,
    ...
    sysMenu.status !== undefined && sysMenu.status !== null
      ...
      : undefined,
    sysMenu.perms
      ? like(sys_menuTable.perms, `%${sysMenu.perms}%`)
      : undefined,
    sysMenu.delFlag
      ? eq(sys_menuTable.del_flag, sysMenu.delFlag)
      : undefined,
  ];
  const result = await db
    .select()
...
export async function insertMenu(menu: SysMenu) {
  const newMenu = await db.insert(sys_menuTable).values({
    path: menu.path ?? "",
    type: menu.type ?? "M",
